            ...
            if let index = stack.pop() {
                ...
                if node.aabb.intersects(aabb) {
                    if node.isLeaf {
                        ...
                        stack.push(node: node.child1)
                        stack.push(node: node.child2)
                    }
                }
            }
        }
        ...
        func dump() {
            ...
            while !n.isEmpty() {
                if let (level, i) = n.pop() {
                    ...
                    if !nodes[i].isLeaf {
                        n.push(node: (level+1, nodes[i].child1))
                        n.push(node: (level+1, nodes[i].child2))
                    }
                }
            }
        }
                    ...
                    while !n.isEmpty() {
                        if let i = n.pop() {
                            if nodes[i].isLeaf {
                                ...
                                n.push(node: nodes[i].child1)
                                n.push(node: nodes[i].child2)
                            }
                        }
                    }
                }
...
fileprivate extension CGPoint {
    func multiply(_ factor: CGFloat) -> Self {
        return .init(x: self.x * factor, y: self.y * factor)
    }
}
